...
namespace Internal {  // DCF77_Second_Decoder
    ...
    void DCF77_Second_Decoder::convolution_binning(const uint8_t tick_data) {
	    ...
	    const bool is_match = (tick_data == DCF77::A0_B0); // there is a match if tick_data = 100ms --> A0_B0
	    uint8_t bin = this->tick; // process current tick

 	    if (bin == 52 || bin == 59) {                // enter here for bin52 or bin59
	      ...
	      and_match = (bin == 52); // 52 is always 7 seconds earlier than 59 ;-)
            ...
            if (bin == this->max_index) prediction_match += is_match;
